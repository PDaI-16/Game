   ...
   void GenerateEnvironmentObjects()
{
    ...
    for (int x = 0; x < width; ++x)
    {
        for (int y = 0; y < height; ++y)
        {
            ...
            // Spawn the portal if it hasn't been spawned yet and the current biome is suitable
            if (!portalSpawned && isSuitableForPortal && Random.value < 0.01f) // Adjust the probability as needed
            {
